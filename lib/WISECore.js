...
var strActionReqTopic = '/wisepaas/<tag>/<clientid>/agentactionreq';	/*Subscrib '/wisepaas/<custom>/<devId>/agentactionreq'e*/
...
/**
 * WISECore - WISECore constructor.
 *  
 * @api public
 * @example var client = new WISECore();
 */
var WISECore = function () {
	this.core_contex = {};

	for (var k in core_contex_t) {
		this.core_contex[k] = core_contex_t[k];
	};

	for (var k in mqtt_ssl_t) {
 ...
 */
WISECore.prototype.core_initialize = function (strClientID, strHostName, strMAC, userdata) {
 ...
 */
WISECore.prototype.core_uninitialize = function () {
 ...
 */
WISECore.prototype.core_tag_set = function (strTag) {
 ...
 */
WISECore.prototype.core_product_info_set = function (strSerialNum, strParentID, strVersion, strType, strProduct, strManufacture) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_account_bind = function (strLoginID, strLoginPW) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_connection_callback_set = function (on_connect, on_lostconnect, on_disconnect, on_msg_recv) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_action_callback_set = function (on_rename, on_update) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_server_reconnect_callback_set = function (on_server_reconnect) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_iot_callback_set = function (on_get_capability, on_start_report, on_stop_report) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_time_tick_callback_set = function (on_get_timetick) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_heartbeat_callback_set = function (on_query_heartbeatrate, on_update_heartbeatrate) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_action_response = function (iCmdid, strSessoinid, bSuccess, strClientid) {
	if (!this.core_contex.bInited) {
		return false;
	}
	...
	var strtopic = strActionAckTopic.replace('<clientid>', strClientid).replace('<tag>', 'device');
	var result = bSuccess ? 'SUCCESS' : 'FALSE';
	var timetick = 0;
 ...
 */
WISECore.prototype.core_heartbeatratequery_response = function (iHeartbeatRate, strSessoinid, strClientid) {
	if (!this.core_contex.bInited) {
		return false;
	}
	if (!this.core_contex.Client) {
 ...
 */
WISECore.prototype.core_tls_set = function (strCaFile, strCaPath, strCertFile, strKeyFile, strCerPasswd) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_tls_psk_set = function (strPsk, strIdentity, strCiphers) {
	if (!this.core_contex.bInited) {
		return false;
	}
 ...
 */
WISECore.prototype.core_connect = function (strServerIP, iServerPort, strConnID, strConnPW) {
	var core_contex = this.core_contex;
	...
	if (this.core_contex.mqtt_ssl.tlstype == 1) { /* To be verified */
		...
		var KEY = fs.readFileSync(this.core_contex.mqtt_ssl.strKeyFile);
		var CERT = fs.readFileSync(this.core_contex.mqtt_ssl.strCertFile);
		var TRUSTED_CA_LIST = fs.readFileSync(this.core_contex.mqtt_ssl.strCaFile);
	...
	this.core_contex.Client = mqtt.connect(options);
 ...
 */
WISECore.prototype.core_disconnect = function (bForce) {
	if (!this.core_contex.bInited) {
		return false;
	}
	if (this.core_contex.Client != null) {
		...
		if (this.core_contex.on_get_timetick != null)
			timetick = this.core_contex.on_get_timetick();
		var msg = GenAgentInfo(this.core_contex, timetick, 0);
		var topic = strInfoAckTopic.replace('<clientid>', this.core_contex.strClientID);
 ...
 */
WISECore.prototype.core_device_register = function () {
	if (!this.core_contex.bInited) {
		return false;
	}
	if (this.core_contex.Client == null) {
	...
	var msg = GenAgentInfo(this.core_contex, timetick, 1);
	var topic = strInfoAckTopic.replace('<clientid>', this.core_contex.strClientID);
 ...
 */
WISECore.prototype.core_heartbeat_send = function () {
	if (!this.core_contex.bInited) {
		return false;
	}
	if (this.core_contex.Client == null) {
	...
	var msg = strHeartbeat.replace('<clientid>', this.core_contex.strClientID);
	var topic = strHeartbeatTopic.replace('<clientid>', this.core_contex.strClientID);
	return this.core_contex.Client.publish(topic, msg);
}
 ...
 */
WISECore.prototype.core_publish = function (strTopic, strPkt, bRetain, iQos, callback) {
	if (!this.core_contex.bInited) {
		return false;
	}
	if (this.core_contex.Client == null) {
 ...
 * @example client.subscribe('topic', 0, console.log);
  */
WISECore.prototype.core_subscribe = function (strTopic, iQos, callback) {
	if (!this.core_contex.bInited) {
		return false;
	}
	if (this.core_contex.Client == null) {
...
function On_Recv(core_contex, topic, payload, packet) {
	...
	switch (root['commCmd']) {
		...
		case CommCmd.wise_server_control_req:
			{
				var iStatusCode = root['content']['response']['statuscode'];
				if(iStatusCode == 4){
		...
		case CommCmd.wise_heartbeatrate_update_req:
			if(core_contex.on_update_heartbeatrate != null) {
				var strSessionID = root['sessionID'];
				var iRate = root['content']['heartbeatrate'];
...
function On_Update(core_contex, root) {
	var strUserName = root['content']['userName'];
	var strPwd = root['content']['pwd'];
	var strSessionID = root['sessionID'];
	var iPort = root['content']['port'];
	var strPath = root['content']['path'];
	var strMD5 = root['content']['md5'];
...
function On_Rename(core_contex, root) {
	var strName = root['content']['devName'];
	var strSessionID = root['sessionID'];
